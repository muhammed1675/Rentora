...
export default async function handler(req, res) {
  ...
  try {
    // merchant_bears_cost: false — the customer bears Korapay's own
    // processing fee (Korapay computes and adds it on the hosted checkout
    // page), instead of it being silently deducted from what Rentora is
    // owed. Defaults to true (merchant bears it) when omitted, which is
    // what every payment through this endpoint was doing before.
    const result = await korapayFetch('/charges/initialize', { method: 'POST', body: JSON.stringify({ reference, amount: numericAmount, currency, redirect_url, customer: { name: customer.name || 'Customer', email: customer.email, phone: customer.phone || undefined }, narration: narration || 'Rentora payment', merchant_bears_cost: false }) });
