...
// The public advertising portal lives on its own subdomain (see App.js's
// hostname-based routing) — every "Advertise here" placeholder must send
// people there, never to rentora.com.ng/advertise.
...
export function AdSlot({ slot, className = '' }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data } = await supabase
        .from('ads')
        .select('id, full_name, business_name, image_url, ad_text, whatsapp_number, link_url, starts_at, ends_at')
        .eq('slot', slot)
  ...
  const destination = safeExternalUrl(ad?.link_url || `https://wa.me/${String(ad?.whatsapp_number || '').replace(/\D/g, '')}`);
  const content = ad ? (
