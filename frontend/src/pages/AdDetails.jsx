...
export default function AdDetails() {
  ...
  const messageBody = ad.message_body || displayText.slice(1).join(' — ');
  ...
  return <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 md:py-14">
    ...
    <article className="overflow-hidden rounded-[28px] border border-border/70 bg-card shadow-sm">
      ...
      <div className="grid gap-8 p-6 sm:p-10 md:grid-cols-[1fr_auto] md:items-end">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">{ad.business_name || 'Rentora advertiser'}</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-foreground sm:text-4xl">{displayText[0] || 'Featured advertisement'}</h1>{messageBody && <p className="mt-4 max-w-2xl whitespace-pre-wrap text-base leading-7 text-muted-foreground">{messageBody}</p>}<p className="mt-5 text-sm text-muted-foreground">Connect directly with this advertiser to learn more.</p></div>
