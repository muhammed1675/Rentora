...
// Korapay-backed bank list + account name resolution.
// Set the secret with: supabase secrets set KORAPAY_SECRET_KEY=...
...
serve(async (req: Request) => {
  ...
  // GET /resolve-bank?list=true  → return Korapay's NG bank list,
  ...
  if (req.method === "GET" && url.searchParams.get("list") === "true") {
    try {
      const res = await fetch(`${KORAPAY_BASE_URL}/misc/banks`, {
  ...
  try {
    ...
    // else here means malformed/oversized input never reaches Korapay's
    ...
    const res = await fetch(`${KORAPAY_BASE_URL}/misc/banks/resolve`, {
      method: "POST",
      headers: {
